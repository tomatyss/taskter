...
def execute_agent_task_async(self, task_id, agent_id):
    ...
    with create_app_context():
        ...
        logger.info(
            f"Starting async execution: Task {task_id} with Agent {agent_id}")
        ...
        try:
            ...
            if result['success']:
                logger.info(
                    f"Successfully completed async execution: Task {task_id}")
                return {
                    'status': 'completed',
                ...
                logger.error(
                    f"Failed async execution: Task {task_id} - {result.get('error', 'Unknown error')}")
                return {
                    'status': 'failed',
            ...
            logger.error(
                f"Exception in async execution: Task {task_id} - {str(e)}")
            ...
            is_retryable = any(
                error in error_str for error in retryable_errors)

            if is_retryable and self.request.retries < self.max_retries:
                logger.warning(
                    f"Retryable error for task {task_id}, attempt {self.request.retries + 1}/{self.max_retries + 1}: {str(e)}")
        ...
        try:
            ...
            for task in pending_tasks:
                ...
                if not agent or not agent.is_active:
                    logger.warning(
                        f"Skipping task {task.id}: Agent {task.assigned_agent_id} not active")
                    continue
                ...
                if running_execution:
                    logger.info(
                        f"Task {task.id} already has running execution {running_execution.id}")
                    continue

                # Start async execution
                logger.info(
                    f"Starting async execution for task {task.id} with agent {task.assigned_agent_id}")
