...
impl ModelProvider for OpenAIProvider {
    ...
    fn build_history(&self, agent: &Agent, user_prompt: &str) -> Vec<Value> {
        if is_responses_model(&agent.model) {
            vec![json!({
                "role": "user",
    ...
    fn request_body(&self, agent: &Agent, history: &[Value], tools: &Value) -> Value {
        if is_responses_model(&agent.model) {
            // Expect history as a list of message objects for Responses
            let mut body = json!({
                "model": agent.model,
    ...
    fn parse_response(&self, v: &Value) -> Result<ModelAction> {
            ...
            for out in output_items {
                if out.get("type").and_then(|x| x.as_str()) == Some("function_call") {
                    let call_id = out
                        ...
                        .and_then(|x| x.as_str())
                        .map(|s| s.to_string())
                        .or_else(|| out.get("id").and_then(|x| x.as_str()).map(|s| s.to_string()));
                    let name = out.get("name").and_then(|x| x.as_str()).unwrap_or("").to_string();
                    ...
                    if !name.is_empty() {
                        return Ok(ModelAction::ToolCall { name, args, call_id });
                    }
                }
                    ...
                    if let Some(content_arr) = out.get("content").and_then(|c| c.as_array()) {
                        for item in content_arr {
                            if item.get("type").and_then(|x| x.as_str()) == Some("tool_call") {
                                let call_id = item.get("id").and_then(|x| x.as_str()).map(|s| s.to_string());
                                let name = item.get("name").and_then(|x| x.as_str()).unwrap_or("").to_string();
                                let args_val = item.get("arguments").cloned().unwrap_or_else(|| json!({}));
                                let args = match args_val {
                                    Value::String(s) => serde_json::from_str::<Value>(&s).unwrap_or(json!({})),
                                    other => other,
                                };
                            ...
                            if item.get("type").and_then(|x| x.as_str()) == Some("output_text") {
                                if let Some(text) = item.get("text").and_then(|x| x.as_str()) {
                                    return Ok(ModelAction::Text { content: text.to_string() });
                                }
                            }
                        }
                    }
        ...
        if let Some(choice) = v.get("choices").and_then(|c| c.as_array()).and_then(|arr| arr.get(0)) {
